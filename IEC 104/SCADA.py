...
def asdu_received_handler(asdu):
    ...
    for element in asdu.get_elements():
        try:
            gvar.iec_values[str(element.get_object_address())
                            ] = element.get_value()
...
def test_client():
    ...
    while True:
        ...
        client = T104Connection(os.environ.get('SERVERIP').encode(
            'UTF-8'), int(os.environ.get('TCPPORT')))
        ...
        with client.connection():
            ...
            # Read single value
            # client.send_read_command(1, 100) # CA,IOA READ IOA = 100
            ...
            # client.send_control_command(cot=CauseOfTransmission.ACTIVATION, ca=1, command=command) # Activation start data transfer -> Answer from Substation: activation confirmation, activation termination when finished
